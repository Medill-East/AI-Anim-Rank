...
function PopulatedRankingWorkspace({ works, methodologyVersion = "v1-auditable-three-source", sourceSnapshotVersion = "2026-07-12", progressRepository }: RankingWorkspaceProps) {
  ...
  const applyInsightStatus = (status: PrivateStatusFilter, label: string) => {
    ...
    rankingControlsRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  return <section ref={workspaceRef} className="ranking-workspace" data-page-jump-target="page-start" aria-label="AnimeRank">
    ...
    <PrivateSummary works={works} records={records} onStatusSelect={applyInsightStatus} onGenreSelect={applyInsightGenre} />
...
function PrivateSummary({ works, records, onStatusSelect, onGenreSelect }: { works: readonly RankedWork[]; records: readonly ProgressRecord[]; onStatusSelect: (status: PrivateStatusFilter, label: string) => void; onGenreSelect: (genre: string) => void }) {
  ...
  return <section className="private-summary" aria-label="我的进度"><div className="summary-heading"><h2>我的进度</h2><p>你的所有偏好仅保存在本地。</p></div><div className="summary-stats"><div className="summary-stat"><span>总收录</span><strong>{works.length}<small> 部</small></strong></div><button type="button" className="summary-stat" onClick={() => onStatusSelect("watched", "已看作品")}><span>已看</span><strong>{watched}<small> 部</small></strong></button><button type="button" className="summary-stat" onClick={() => onStatusSelect("unwatched", "未看作品")}><span>当前完成度</span><strong>{completion}<small>%</small></strong></button><div className="summary-stat" data-progress-insight="favorite-studio"><span>常看制作</span><strong className="studio-stat">{insights.topStudio?.label ?? "—"}</strong><small>{insights.topStudio ? `${insights.topStudio.count} 部已看作品` : "开始标记后显示"}</small></div></div><section className="progress-insights" aria-label="进度洞察"><section className="completion-insight" data-progress-insight="completion"><button type="button" className="completion-ring" aria-label={`查看 ${works.length - watched} 部未看作品`} onClick={() => onStatusSelect("unwatched", "未看作品")} style={{ background: `conic-gradient(var(--accent) ${insights.completion * 3.6}deg, var(--surface-hover) 0deg)` }}><span>{insights.completion}%</span><small>完成</small></button><div><h3>完成度</h3><p>已看 {insights.watchedCount} / {works.length} 部 · 已评价 {reviewed} · 推荐 {recommended} · 不感兴趣 {notInterested}</p><button type="button" className="insight-link" onClick={() => onStatusSelect("unwatched", "未看作品")}>查看未看作品</button></div></section><section className="preference-insight" data-progress-insight="genres"><div className="insight-heading"><h3>观看偏好</h3><span>{insights.watchedCount === 0 ? "开始标记后解锁" : "按已看作品统计"}</span></div>{insights.topGenres.length > 0 ? <div className="genre-insights">{insights.topGenres.map((genre) => <button key={genre.label} type="button" className="genre-insight" onClick={() => onGenreSelect(genre.label)}><span>{genre.label}</span><i><b style={{ width: `${genre.percentage}%` }} /></i><em>{genre.count} 部</em></button>)}</div> : <p className="insight-empty">还没有已看作品，先从榜单里标记一部吧。</p>}<div className="insight-footer"><span>常看制作</span><strong>{insights.topStudio ? `${insights.topStudio.label} · ${insights.topStudio.count} 部` : "—"}</strong><span>推荐</span><strong>{insights.recommendedCount} 部</strong></div></section></section></section>;
}
