...
`release` does not read `candidate-review.json`. It rebuilds from the captured AniList/Jikan files plus the checked-in reviewed Bangumi mappings, validates every complete candidate's positive, globally unique AniList/MAL/Bangumi IDs, source identity, snapshot fields and vote eligibility, then requires at least 300 eligible entries. It deterministically publishes the top 300 by composite score descending, then AniList ID ascending. Candidate review JSON is only a human review artifact. Release uses an adjacent temporary file then rename, so a refusal leaves `src/data/ranking.json` unchanged.
