...
test("demotes duplicate identities before dry-run audit output", async () => {
  ...
  try {
    ...
    assert.deepEqual(JSON.parse(await readFile(join(directory, "resolution.json"), "utf8")).accepted.length, 1);
  } finally { await rm(directory, { recursive: true, force: true }); }
});
...
test("does not persist the access token and paces bounded retries", async () => {
  ...
  try {
    ...
    assert.equal(`${await readFile(join(directory, "resolution.json"), "utf8")}\n${await readFile(join(directory, "exceptions.json"), "utf8")}`.includes(token), false);
  } finally { await rm(directory, { recursive: true, force: true }); }
});
...
test("does not publish resolver artifacts when apply validation fails", async () => {
  ...
  try {
    ...
    await assert.rejects(readFile(join(directory, "resolution.json"), "utf8"));
